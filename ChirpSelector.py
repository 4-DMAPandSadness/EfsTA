import numpy as np
from PyQt5.QtWidgets import QWidget, QPushButton, QGridLayout, QTextBrowser
from matplotlib.widgets import SpanSelector
from matplotlib.backends.backend_qt5agg import FigureCanvasQTAgg as FigureCanvas
import matplotlib.pyplot as plt

class ChirpSelector(QWidget):
    ...
    def initUI(self):
        ...
        text = ("Please select the data range to use for the chirp fitting. "
                "You can select one or more areas with your mouse by "
                "left-clicking on the figure, holding and draging your mouse. "
                "Click 'Show' to show all selected areas, "
                "'Undo' to remove the last selected area, "
                "'Reset' to remove all selected areas and "
                "'Continue' once you are finished to start the fitting process."
                "ONLY shown range(s) will  be taken into account."
                "\n"
                "\n"
                "1. Use the span selector to select the spectral range(s).\n"
                "2. Click 'Show' to show the selected range(s).\n"
                "3. (Opt) Click 'Undo' or 'Reset' to change range(s).\n"
                "4. Click 'Show' again and then 'Continue' to start.\n"
                )
